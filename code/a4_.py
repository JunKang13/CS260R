...
Jiarui Han(jh2379) and Stephanie Balbuena(seb359)
10/27/2023
"""
...
def draw_spiral(w, side, ang, n, sp):
    ...
    assert is_valid_length(side * n), \
        ('side is not a valid length', side * n)
    assert is_valid_iteration(n), \
        report_error('n is not a valid number of iterations', n)
    assert is_valid_speed(sp), report_error('sp is not a valid speed', sp)
...
def draw_spiral_helper(t, side, ang, n, sp):
    ...
    assert is_valid_length(side), \
        report_error('side is not a valid length', side)
    assert is_valid_length(side * n), \
        report_error('side is not a valid length', side * n)
    assert is_valid_iteration(n), \
        report_error('n is not a valid number of iterations', n)
    assert is_valid_speed(sp), report_error('sp is not a valid speed', sp)
...
def radiate_petals(w, radius, width, n, sp):
    ...
    # ARE THESE ALL OF THE PRECONDITIONS?
    assert is_window(w), report_error('w is not a valid window', w)
    assert is_valid_length(radius), \
        report_error('radius is not a valid length', radius)
    assert is_valid_length(width), \
        report_error('width is not a valid length', width)
    assert is_valid_speed(sp), report_error('sp is not a valid speed', sp)
    assert is_number(n), report_error('n is not a valid number', n)
    assert type(n) == int, report_error('n is not a integer', n)
    assert n >= 2, report_error('n is not >= 2', n)
    # HINT: w.clear() clears window.
    # HINT: Set the visible attribute to False at the end, and remember to flush
    w.clear()
    t = Turtle(w)
...
def radiate_petals_helper(t, radius, width, n, sp):
    ...
    assert (type(n) == int and n >= 2), \
        report_error('n is an invalid # of petals', n)
    assert is_valid_speed(sp), report_error('sp is not a valid speed', sp)
...
def draw_diamond(t, length, width):
    ...
    assert is_valid_length(length), \
        report_error('length is not a valid length', length)
    assert is_valid_length(width), \
        report_error('width is not a valid length', width)
...
def hexaflake(w, side, d, sp):
    ...
    assert is_valid_speed(sp), report_error('sp is not a valid pen speed', sp)
    # HINT: w.clear() clears window.
    ...
    p.solid = False
    p.edgecolor = 'gray'
    p.fillcolor = "sky blue"

    hexaflake_helper(p, 0, 0, side, d)
    p.flush()
...
def hexaflake_helper(p, x, y, side, d):
    ...
    if d == 0:
        ...
        os = [math.pi / 3 * i for i in range(6)]
        nxt = [(2 * side / 3 * math.cos(i), 2 * side / 3 * math.sin(i)) for i in os]
        for j in nxt:
            hexaflake_helper(p, x + j[0], y + j[1], side / 3, d - 1)
        hexaflake_helper(p, x, y, side / 3, d - 1)
